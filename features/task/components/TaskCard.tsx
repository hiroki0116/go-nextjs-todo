...
const TaskCard = ({ task }: { task: ITask }) => {
  ...
  const handleCompleteTask = async () => {
    try {
      ...
      setTasks([
        ...tasks.map((curr) =>
          curr._id === task._id ? { ...curr, completed: !curr.completed } : curr
        ),
      ]);
    ...
    <div
      className={`flex justify-between rounded border px-3 py-2 mr-3 text-primary relative ${
        task.completed
          ...
          : "hover:bg-slate-800/90 transition duration-300 transform ease-in-out"
      }`}
    >
      <div
        className={`text-left ${
          !task.completed ? "animate-pulse text-white" : "text-primary"
        }`}
      >
        {task.title}
      </div>
